...
const typingtext=document.querySelector(".text-para p");
const input=document.querySelector(".input-field");
const mistakesp=document.querySelector(".mistakes span");
...
function randomPara(){
    ...
    paragraphs[randomIndex].split("").forEach(e => {
        typingtext.innerHTML+=`<span>${e}</span>`;
    });
    typingtext.querySelectorAll("span")[0].classList.add("active")
    typingtext.addEventListener("keydown", ()=> input.focus());
...
function inittyping(){
    ...
    let wpm=Math.round((((charindex-mistakes)/5)/(totaltime-lefttime))*60);
    wpm = wpm<0 || !wpm || wpm===Infinity ? 0:wpm;
    cpmtag.innerText=charindex-mistakes;
    wpmtag.innerText=wpm;
    mistakesp.innerText=mistakes;
